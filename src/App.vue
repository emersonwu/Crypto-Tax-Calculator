  ...
  methods: {
    ...
    jsonToTrades(json){
      ...
      json.map(function(row){
        var obj = {}
        obj["assetName"] = row["Asset"]
        obj["event"] = row["Event"]
        obj["amount"] = row["Amount"]
        obj["value"] = row["AUD Value"]
        obj["fee"]= row["Fee AUD Value"]
    ...
    csvToJSON(csv){
      ...
      vm.parse_header = lines[1].split(",") 
      lines[1].split(",").forEach(function (key) {
        vm.sortOrders[key] = 1
      })
      
      lines.map(function(line, indexLine){
        if (indexLine < 2) return // Jump header line
